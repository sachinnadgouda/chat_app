...
describe("Chat Server",function(){
  ...
  it('Should broadcast new user once they connect',function(done){
    ...
    client.on('connect',function(data){
      client.emit('connection handle',chatUser1);
    });

    client.on('new user',function(usershandle){
      usershandle.should.be.type('string');
      usershandle.should.equal(chatUser1.handle + " has joined.");
  ...
  it('Should broadcast new user to all users', function(done){
    ...
    client1.on('connect', function(data){
      client1.emit('connection handle', chatUser1);
      ...
      client2.on('connect', function(data){
        client2.emit('connection handle', chatUser2);
      });

      client2.on('new user', function(usershandle){
        usershandle.should.equal(chatUser2.handle + " has joined.");
        client2.disconnect();
      });
    ...
    var numUsers = 0;
    client1.on('new user', function(usershandle){
      numUsers += 1;
  ...
  it('Should be able to send private messages', function(done){
    var client1, client2, client3;
    var message = {to: chatUser1.handle, txt:'Private Hello World'};
    var messages = 0;
    ...
    var checkPrivateMessage = function(client){
      client.on('private message', function(msg){
        ...
        msg.from.should.equal(chatUser3.handle);
        messages++;
        if(client === client1){
    ...
    client1.on('connect', function(data){
      ...
      client2.on('connect', function(data){
        ...
        client3.on('connect', function(data){
          client3.emit('connection handle', chatUser3);
